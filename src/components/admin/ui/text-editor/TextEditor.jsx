...
const RichTextEditor = ({ value, onChange, onImageUpload }) => {
  ...
  const saveToHistory = useCallback(
    (contentToSave = null) => {
      ...
      const currentContent =
        contentToSave !== null ? contentToSave : editorRef.current.innerHTML;
  ...
  const execCommand = (command, value = null) => {
    ...
    if (
      ...
      if (value === "h2") {
        const newBlock =
          currentBlock === "h2" || currentBlock === "" ? "p" : "h2";
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener("selectionchange", checkActiveCommands);
  ...
  const insertImage = async () => {
    ...
    if (imageFile && onImageUpload) {
      ...
      setImageError("");
      try {
        finalUrl = await onImageUpload(imageFile);

        if (!finalUrl) {
          setImageError("Image upload failed. No URL returned.");
          return;
        }
      } catch (err) {
        setImageError(err.message || "Image upload error during transfer.");
        return;
      } finally {
  ...
  const handleImageUrlChange = (e) => {
    setImageUrl(e.target.value); // Set the manual URL
  ...
  const insertTable = () => {
    ...
    for (let i = 0; i < tableRows; i++) {
      ...
      for (let j = 0; j < tableCols; j++) {
        tableHTML +=
          '<td style="border: 1px solid #ddd; padding: 8px; min-width: 50px;">&nbsp;</td>';
      }
      tableHTML += "</tr>";
    }
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "Enter" && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
      ...
      const currentBlock = document
        .queryCommandValue("formatBlock")
        .toLowerCase();
      if (currentBlock === "blockquote" || currentBlock === "pre") {
  ...
  const cleanHtmlContent = useCallback((html) => {
    ...
    if (!cleaned.match(/^\s*<(p|h[1-6]|ul|ol|table|blockquote|pre|img)/i)) {
      cleaned = `<p>${cleaned}</p>`;
    }
    ...
    <div className="w-full flex flex-col bg-slate-50">
      <div className="p-3 shadow-md bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="flex flex-wrap items-center gap-1 max-w-7xl mx-auto">
          ...
          <ToolbarButton
            ...
            icon={Heading}
            title={
              activeCommands.subheading
                ? "Change to Paragraph"
                : "Change to Subheading (H2)"
            }
          ...
          <div className="relative">
            ...
            {showColorPicker && (
              <div
                className="absolute top-12 left-0 p-2 bg-white rounded-lg shadow-xl z-50 flex gap-1 flex-wrap w-48 border border-slate-200"
      ...
      <style global jsx>{`
        ...
        .editor-styles p {
          font-size: 16px;
          margin: 12px 0;
        } /* Added explicit P styling for consistency */
      `}</style>
      ...
      {showLinkModal && (
        ...
        >
          <div
            className="bg-white p-6 w-96 rounded-lg shadow-2xl"
          ...
          >
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Leave empty if you have text selected
              </p>
            </div>
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="px-4 py-2 text-slate-700 bg-slate-200 rounded-lg hover:bg-slate-300 transition duration-150"
              >
                Cancel
              </button>
              <button
      ...
      {showImageModal && (
        ...
        >
          ...
          >
            ...
            {imageError && (
              <div className="text-xs bg-red-100 text-red-700 p-2 rounded border border-red-300 mb-3">
                {imageError}
              </div>
            )}
            ...
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full p-2 border border-slate-300 rounded-md focus:outline-none focus:border-blue-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
              />
              {imageLoading && (
              ...
              {imageFile && (
                <p className="text-xs text-slate-500 mt-1">
                  Ready to insert: **{imageFile.name}**
                </p>
              )}
            </div>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                value={imageUrl}
                onChange={handleImageUrlChange} // <--- CORRECT HANDLER
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={insertImage}
                disabled={imageLoading || (!imageUrl && !imageFile)} // Disabled if loading or neither file/URL is present
                className="px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition duration-150 disabled:opacity-60"
              >
                {imageLoading ? (
